...
def preprocessing(df):
    ...
    # 輸出
    df_majority.to_csv("./inter_data/majority_class.csv", index=False, encoding="utf-8-sig")
    print(f"✅ 已輸出合併檔案: ./inter_data/majority_class.csv，共 {len(df_majority)} 筆資料")

    df_minority.to_csv("./inter_data/minority_class.csv", index=False, encoding="utf-8-sig")
    print(f"✅ 已輸出合併檔案: ./inter_data/minority_class.csv，共 {len(df_minority)} 筆資料")
