...
def compute_wolf_correlation(wolf_vector, data):
    ...
    # 移除方差為 0 的特徵
    selected_data = selected_data[:, np.std(selected_data, axis=0) > 0]
    ...
    corr_matrix = np.nan_to_num(corr_matrix, nan=0.0)  # 把 NaN 設為 0
...
if __name__ == "__main__":
    ...
    df = pd.read_csv("./extra_dataset/combined_oversampling_without_GAMO.csv")
    # 生成一個假資料集
